...
namespace Exam.Models
{
	...
	public class Blob : IBlob
	{
		...
		public Blob(string name, int health, int damage, IBehavior blobBehavior, IAttack blobAttack)
		{
			...
			this.BlobAttack = blobAttack;

			this.IsAlive = true;
			this.BlobBehavior.HasBeenTriggered = false;
		}
		...
		public bool IsAlive
		{
			...
			set
			{
				this.LifeStateChange?.Invoke(this, new LifeReportEventArgs("Name", this.isAlive, value));
				this.isAlive = value;
			}
		}
		...
		{
			if (this.IsAlive && targetBlob.IsAlive)
			{
				this.BlobAttack.Attack(this, targetBlob);
		...
		{
			this.BlobBehavior.EndTurnAction(this);
		}
	}
}